...
public interface RestShorts {
	...
	String REMOVEALLFROMUSER = "/removeallfromuser";
	...
	@DELETE
	@Path("/{" + USER_ID + "}" + REMOVEALLFROMUSER)
	void removeAllFromUser(@PathParam(USER_ID) String userId, @QueryParam(PWD) String password);


	@POST
    @Path("/{" + BLOB_ID + "}" + EXISTS)
    void blobStillExist(@PathParam(BLOB_ID) String blobId) throws InterruptedException;
}
